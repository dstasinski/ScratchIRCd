...
The existing `protocol_integration` CTest is the Milestone 1 TCP smoke test. It starts a real daemon, registers multiple clients, exercises channel messaging and operator/identity boundaries, and stops the server. The `registration_input_integration` test covers fragmented and coalesced TCP input plus framing attacks. The `registration_state_integration` test covers automatic MOTD delivery, missing-MOTD termination, command ordering, immutable USER state, RFC1459 nickname collisions, reserved service nicknames, retry behavior, and single welcome emission. The `connection_lifecycle_integration` test repeatedly exercises pre-registration drops, orderly QUIT, abrupt registered disconnects, membership cleanup, empty-channel destruction, and continued service to a survivor. The `connection_limit_integration` test covers both the global client ceiling and per-IP limits, capacity release, literal exemptions, trusted WebIRC gateway handling, end-user enforcement, and canonical IPv6 identity. The `ping_timeout_integration` test verifies activity resets the liveness interval, exact PONG-token matching, rejection of unrelated traffic as a substitute response, timeout disconnect reason, and continued service to peers. The `message_routing_integration` test covers direct, self, channel, canonical-target and displayed-host routing; silent NOTICE failures; and the no-spoof CTCP VERSION response path, including the pre-response JOIN/message restriction and the direct IRCop/netadmin exception. The `tls_integration` test covers successful TLS registration and `+z`, plaintext denial on a TLS-only channel, malformed-handshake rejection, silent-handshake timeout, continued service after TLS failures, and process shutdown with active plain and TLS clients. The `startup_failure_integration` test proves malformed and invalid configuration, incomplete or unreadable TLS credentials, unusable database paths, and listener conflicts all fail closed with actionable diagnostics. The `sendq_pressure_integration` test stops reading from a deliberately constrained TCP client, drives it through the configured 4 KiB SendQ ceiling, verifies only that slow reader is disconnected, proves healthy peers remain responsive during pressure, and confirms the released nickname and client slot can be reused. The `ipv6_integration` test binds an actual IPv6-only loopback listener, registers two AF_INET6 clients, exercises channel and private messaging plus PING/PONG, and shuts down with a live IPv6 client. The `soak_runner_smoke_integration` test validates the operational soak harness with no-spoof and CTCP VERSION enabled. The `soak_release_gate_integration` test proves a short smoke run cannot be recorded as release qualification. A recorded 12-24 hour invocation with both `passed` and `release_qualified` set remains an operational release gate.
