...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="scratchircd-identsec-") as td:
        ...
        try:
            ...
            # +r is legitimate from NickServ, +t is legitimate from the
            # earlier SETHOST, and every registered client receives +x.
            # SAMODE must not manufacture oper/TLS/WebIRC provenance modes or
            # remove the existing vhost and automatic cloak modes.
            assert all(letter in token for letter in "rtx"), token
            assert "o" not in token and "z" not in token and "V" not in token, token
        finally:
