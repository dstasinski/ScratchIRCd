...
CMD(admin); CMD(authenticate); CMD(away); CMD(cap); CMD(chanserv); CMD(chathistory); CMD(csdrop); CMD(csinfo); CMD(csset); CMD(deaf); CMD(die); CMD(eline); CMD(flash); CMD(geoban); CMD(globops); CMD(identify); CMD(info); CMD(invite); CMD(ison); CMD(join); CMD(kick); CMD(kill); CMD(kline); CMD(knock); CMD(links); CMD(list); CMD(locops); CMD(lusers); CMD(memoserv); CMD(msinfo); CMD(mspurge); CMD(mode); CMD(motd); CMD(mute); CMD(names); CMD(nick); CMD(nickserv); CMD(notice); CMD(nsdrop); CMD(nsinfo); CMD(nsset); CMD(oper); CMD(operadd); CMD(operdel); CMD(operlist); CMD(operset); CMD(part); CMD(pass); CMD(ping); CMD(pmstats); CMD(pong); CMD(privmsg); CMD(quit); CMD(rehash); CMD(restart); CMD(rules); CMD(sajoin); CMD(samode); CMD(sapart); CMD(sethost); CMD(setident); CMD(setname); CMD(silence); CMD(snotice); CMD(stats); CMD(tagmsg); CMD(time); CMD(topic); CMD(ungeoban); CMD(user); CMD(userhost); CMD(userip); CMD(version); CMD(wallops); CMD(watch); CMD(webirc); CMD(who); CMD(whois); CMD(whowas); CMD(zline);
#undef CMD
