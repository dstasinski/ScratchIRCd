/**
 * @file chanserv_db.c
 * @brief SQLite persistence for registered ChanServ channels.
 */
...
static int update_text(ChanServDb *db, const char *name, const char *column,
                       const char *value) {
    char sql[192];
    sqlite3_stmt *stmt = NULL;
    int rc;
    ...
    (void)snprintf(sql, sizeof(sql),
                   "UPDATE channels SET %s=?1,updated_at=unixepoch() WHERE name=?2", column);
...
int chanserv_db_set_description(ChanServDb *db, const char *name,
                                const char *description) {
    return update_text(db, name, "description", description);
}

int chanserv_db_set_founder(ChanServDb *db, const char *name,
                            const char *founder) {
...
int chanserv_db_delete(ChanServDb *db, const char *name) {
    ...
    int rc;
    if (db == NULL || db->db == NULL || name == NULL) return -1;
    if (sqlite3_prepare_v2(db->db, "DELETE FROM channels WHERE name=?1",
                           -1, &stmt, NULL) != SQLITE_OK) return -1;
    sqlite3_bind_text(stmt, 1, name, -1, SQLITE_TRANSIENT);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
...
int chanserv_db_list_enabled(ChanServDb *db, char *buffer, size_t size) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        int written = snprintf(buffer + used, size - used, "%s%s", used ? "," : "", name);
