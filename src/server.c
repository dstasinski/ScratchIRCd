...
static int add_listeners(Server *server, const char *port, int use_tls) {
    ...
    for (candidate = result; candidate != NULL && server->listener_count < IRCD_MAX_LISTENERS; candidate = candidate->ai_next) {
        ...
        if (set_nonblocking(fd) != 0 || bind(fd, candidate->ai_addr, candidate->ai_addrlen) != 0 || listen(fd, IRCD_LISTEN_BACKLOG) != 0) {
            close(fd);
            continue;
        }
...
static int init_tls(Server *server) {
    ...
    if (server->config.tls_cert_file[0] == '\0' || server->config.tls_key_file[0] == '\0') {
    ...
    if (SSL_CTX_set_min_proto_version(server->tls_ctx, TLS1_2_VERSION) != 1 ||
        SSL_CTX_use_certificate_chain_file(server->tls_ctx, server->config.tls_cert_file) != 1 ||
        SSL_CTX_use_PrivateKey_file(server->tls_ctx, server->config.tls_key_file, SSL_FILETYPE_PEM) != 1 ||
...
static void accept_clients(Server *server, int listen_fd, int use_tls) {
    for (;;) {
        ...
        if (peer_ip(&address, address_length, ip, sizeof(ip)) != 0) {
            snotice_broadcast(server, SNOTICE_FLOOD, "Connection rejected: unable to determine peer IP");
            close(fd); continue;
        }
        if (server->client_count >= server->config.max_clients) {
            snotice_broadcast(server, SNOTICE_FLOOD, "Connection rejected from %s: global client limit reached (%zu/%zu)", ip, server->client_count, server->config.max_clients);
            close(fd); continue;
        }
        if (server_connection_limit_reached(server, ip, NULL)) {
            snotice_broadcast(server, SNOTICE_FLOOD | SNOTICE_SECURITY, "Connection rejected from %s: per-IP concurrent connection limit reached (%u)", ip, server->config.max_connections_per_ip);
            close(fd); continue;
        }
        if (ensure_client_capacity(server) != 0 || set_nonblocking(fd) != 0) {
            snotice_broadcast(server, SNOTICE_FLOOD, "Connection rejected from %s: server resource allocation/setup failure", ip);
            close(fd); continue;
        }
        client = client_create(fd, ++server->next_client_id, ((struct sockaddr *)&address)->sa_family, ip);
        if (client == NULL) {
            snotice_broadcast(server, SNOTICE_FLOOD, "Connection rejected from %s: client allocation failure", ip);
            close(fd); continue;
        }
        if (use_tls) {
            if (server->tls_ctx == NULL || (client->ssl = SSL_new(server->tls_ctx)) == NULL) {
                snotice_broadcast(server, SNOTICE_SECURITY, "TLS connection setup failed from %s", ip);
                client_free(client); close(fd); continue;
            }
        ...
        if (dns_resolver_submit(&server->dns, client->id, client->address_family, client->real_ip) != 0) {
            client->dns_state = CLIENT_DNS_FAILED;
            snotice_broadcast(server, SNOTICE_DNS, "FCrDNS submission failed for %s", client->real_ip);
...
static int process_buffered_lines(Server *server, Client *client) {
    for (;;) {
        ...
        if (line_length > IRC_LINE_CONTENT_MAX || memchr(client->inbuf, '\0', raw_length) != NULL || memchr(client->inbuf, '\r', line_length) != NULL) {
...
static int read_client(Server *server, Client *client) {
    ...
    if (client->inbuf_len >= sizeof(client->inbuf) - 1U) {
        snotice_broadcast(server, SNOTICE_SECURITY | SNOTICE_FLOOD, "Input buffer exhausted by %s", client->real_ip);
        return 1;
    }
...
Client *server_find_client_by_id(Server *server, uint64_t id) {
    ...
    for (index = 0U; index < server->client_count; ++index) if (server->clients[index]->id == id) return server->clients[index];
    return NULL;
}
...
static void drain_dns_results(Server *server) {
    DnsResult result;
    while (dns_resolver_read_result(&server->dns, &result) == 1) handle_dns_result(server, &result);
}
...
static void drain_dnsbl_results(Server *server) {
    DnsblResult result;
    while (dnsbl_resolver_read_result(&server->dnsbl, &result) == 1) handle_dnsbl_result(server, &result);
}
    ...
    while (index < server->client_count) {
        ...
        if (!client->registered && now - client->signon_time >= (time_t)server->config.registration_timeout_seconds) {
            snotice_broadcast(server, SNOTICE_FLOOD,
                              "Registration timeout for %s (nick=%s cap=%s sasl_state=%d)",
                              client->real_ip, client->nick[0] != '\0' ? client->nick : "*",
                              client->cap_negotiating ? "yes" : "no", (int)client->sasl_state);
            client_sendf(client, ":%s ERROR :Registration timeout", server->config.server_name);
            server_disconnect(server, client, "Registration timeout");
            continue;
        }
        if (server->config.nospoof_enabled && client->nospoof_started && !client->nospoof_verified && client->nospoof_deadline <= now) {
            snotice_broadcast(server, SNOTICE_SECURITY, "No-spoof PING timeout for %s (nick=%s)", client->real_ip, client->nick[0] != '\0' ? client->nick : "*");
        ...
        if (client->dns_state == CLIENT_DNS_PENDING && client->dns_deadline <= now) {
            ...
            client->real_host[0] = '\0';
            snotice_broadcast(server, SNOTICE_DNS, "FCrDNS timeout for %s", client->real_ip);
            if (client->tls_state != CLIENT_TLS_HANDSHAKE) client_sendf(client, NOTICE_FAILDNS, server->config.server_name);
            command_maybe_register(server, client);
        }
        if (client->dnsbl_state == CLIENT_DNSBL_PENDING && client->dnsbl_deadline <= now) {
            client->dnsbl_state = CLIENT_DNSBL_TIMEOUT;
            snotice_broadcast(server, SNOTICE_DNS, "DNSBL lookup timeout for %s", client->real_ip);
            command_maybe_register(server, client);
        }
        ++index;
    }
...
int server_init(Server *server, const ServerConfig *config) {
    ...
    if (hash_init(&server->clients_by_nick, IRCD_CLIENT_HASH_BUCKETS) != 0 || hash_init(&server->channels_by_name, IRCD_CHANNEL_HASH_BUCKETS) != 0) {
        server_destroy(server); return -1;
    }
    if (channel_log_init(server) != 0 || init_tls(server) != 0 || dns_resolver_init(&server->dns) != 0 || dnsbl_resolver_init(&server->dnsbl) != 0 || geoip_init(&server->geoip, server->config.geoip_city_db, server->config.geoip_asn_db) != 0 || make_listeners(server) != 0) {
        server_destroy(server); return -1;
    }
    return 0;
}
...
void server_disconnect(Server *server, Client *client, const char *reason) {
    ...
    if (client->registered) (void)snprintf(quit_message, sizeof(quit_message), ":%s!%s@%s QUIT :%s\r\n", client->nick, client->user, client->display_host, quit_reason);
...
void server_run(Server *server) {
    ...
    while (!server->restart_requested) {
        ...
        if (poll_fds == NULL || (server->client_count > 0U && snapshot == NULL)) {
            snotice_broadcast(server, SNOTICE_FLOOD, "Event-loop allocation failure; stopping server loop");
...
void server_destroy(Server *server) {
    ...
    if (server->listen_fds != NULL) {
        ...
        free(server->listen_fds); server->listen_fds = NULL;
    }
