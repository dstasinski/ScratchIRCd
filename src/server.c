...
void server_disconnect(Server *server, Client *client, const char *reason) {
    ...
    while (client->channels != NULL) { Channel *channel = client->channels->channel; if (client->registered) { channel_log_quit(server, channel, client, quit_reason); channel_broadcast(channel, client, quit_message); } channel_remove_client(channel, client); server_remove_channel_if_empty(server, channel); }
