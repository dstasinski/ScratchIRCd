...
void nospoof_start(Server *server,Client *client){if(server==NULL||client==NULL||!server->config.nospoof_enabled||client->nospoof_started||client->nick[0]=='\0')return;hex_cookie(client->nospoof_cookie,sizeof(client->nospoof_cookie));if(client->nospoof_cookie[0]=='\0')return;client->nospoof_started=1;client->nospoof_deadline=time(NULL)+(time_t)server->config.nospoof_timeout_seconds;client_sendf(client,"PING :%s",client->nospoof_cookie);client_sendf(client,":%s PRIVMSG %s :\001VERSION\001",server->config.server_name,client->nick);client->version_requested=1;}
void nospoof_request_website(Server *server,Client *client){if(server==NULL||client==NULL||!server->config.nospoof_enabled||!client->webirc.active||client->website_requested||client->nick[0]=='\0')return;client_sendf(client,":%s PRIVMSG %s :\001WEBSITE\001",server->config.server_name,client->nick);client->website_requested=1;}
