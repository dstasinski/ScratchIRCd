...
static int valid_numeric_ip(const char *ip) {
    struct in_addr v4;
    struct in6_addr v6;
...
static int valid_cloak_prefix(const char *value) {
    ...
    if (value == NULL || *value == '\0' || strlen(value) > IRCD_CLOAK_PREFIX_MAX)
        return 0;
    for (; *p != '\0'; ++p) {
...
void runtime_config_defaults(ServerConfig *config) {
    ...
    (void)copy_value(config->operators_db, sizeof(config->operators_db),
                     IRCD_DEFAULT_OPERATORS_DB);
    (void)copy_value(config->bans_db, sizeof(config->bans_db), IRCD_DEFAULT_BANS_DB);
    (void)copy_value(config->nickserv_db, sizeof(config->nickserv_db),
                     IRCD_DEFAULT_NICKSERV_DB);
    (void)copy_value(config->chanserv_db, sizeof(config->chanserv_db),
                     IRCD_DEFAULT_CHANSERV_DB);
    (void)copy_value(config->memoserv_db, sizeof(config->memoserv_db),
                     IRCD_DEFAULT_MEMOSERV_DB);
    (void)copy_value(config->history_db, sizeof(config->history_db),
                     IRCD_DEFAULT_HISTORY_DB);
    (void)copy_value(config->geoip_city_db, sizeof(config->geoip_city_db),
                     IRCD_DEFAULT_GEOIP_CITY_DB);
    (void)copy_value(config->geoip_asn_db, sizeof(config->geoip_asn_db),
                     IRCD_DEFAULT_GEOIP_ASN_DB);
    (void)copy_value(config->sendmail_path, sizeof(config->sendmail_path),
                     IRCD_DEFAULT_SENDMAIL_PATH);
    (void)copy_value(config->netadmin_hostmask, sizeof(config->netadmin_hostmask), "*!*@*");
    (void)copy_value(config->kline_default_reason, sizeof(config->kline_default_reason),
                     IRCD_DEFAULT_KLINE_REASON);
    (void)copy_value(config->zline_default_reason, sizeof(config->zline_default_reason),
                     IRCD_DEFAULT_ZLINE_REASON);
    (void)copy_value(config->cloak_prefix, sizeof(config->cloak_prefix),
                     IRCD_DEFAULT_CLOAK_PREFIX);
    ...
    config->history_retention_days = IRCD_DEFAULT_HISTORY_RETENTION_DAYS;
    config->history_max_rows = IRCD_DEFAULT_HISTORY_MAX_ROWS;
    config->channel_log_queue_max_rows = IRCD_DEFAULT_CHANNEL_LOG_QUEUE_MAX_ROWS;
    config->memoserv_quota = IRCD_DEFAULT_MEMOSERV_QUOTA;
    config->memoserv_sender_quota = IRCD_DEFAULT_MEMOSERV_SENDER_QUOTA;
    config->memoserv_retention_days = IRCD_DEFAULT_MEMOSERV_RETENTION_DAYS;
    config->nickserv_registrations_per_ip = IRCD_DEFAULT_NICKSERV_REGISTRATIONS_PER_IP;
    config->nickserv_registration_window_seconds =
        IRCD_DEFAULT_NICKSERV_REGISTRATION_WINDOW_SECONDS;
    ...
    config->nospoof_timeout_seconds = IRCD_DEFAULT_NOSPOOF_TIMEOUT_SECONDS;
}
...
static int add_webirc_gateway(ServerConfig *config, const char *value) {
    ...
    if (config->webirc_gateway_count >= IRCD_MAX_WEBIRC_GATEWAYS ||
        strlen(value) >= sizeof(copy))
        return -1;

    (void)snprintf(copy, sizeof(copy), "%s", value);
    ip = strtok(copy, " \t");
    password = strtok(NULL, " \t");
    if (ip == NULL || password == NULL || strtok(NULL, " \t") != NULL ||
        !valid_numeric_ip(ip))
        return -1;

    gateway = &config->webirc_gateways[config->webirc_gateway_count];
    if (copy_value(gateway->ip, sizeof(gateway->ip), ip) != 0 ||
        copy_value(gateway->password, sizeof(gateway->password), password) != 0)
        return -1;
    ++config->webirc_gateway_count;
    return 0;
}

static int add_connection_limit_exempt_ip(ServerConfig *config, const char *value) {
    char *dest;

    if (config->connection_limit_exempt_ip_count >= IRCD_MAX_CONNECTION_LIMIT_EXEMPT_IPS ||
        !valid_numeric_ip(value))
        return -1;

    dest = config->connection_limit_exempt_ips[config->connection_limit_exempt_ip_count];
    if (copy_value(dest, IRC_IP_MAX + 1U, value) != 0) return -1;
    ++config->connection_limit_exempt_ip_count;
    return 0;
}
...
static int add_dnsbl(ServerConfig *config, const char *value) {
    ...
    if (config->dnsbl_count >= IRCD_MAX_DNSBLS || strlen(value) >= sizeof(copy))
        return -1;
    ...
    if (copy_value(entry->name, sizeof(entry->name), name) != 0 ||
        copy_value(entry->zone, sizeof(entry->zone), zone) != 0)
        return -1;
    ++config->dnsbl_count;
    return 0;
}
...
static int set_bool(const char *value, int *field) {
...
#define STRING_OPTION(name, field) \
    ...
    if (strcmp(key, "cloak_prefix") == 0)
        return valid_cloak_prefix(value)
                   ? copy_value(config->cloak_prefix, sizeof(config->cloak_prefix), value)
                   : -1;
    if (strcmp(key, "cloak_key") == 0) {
        if (*value != '\0' && strlen(value) < 16U) return -1;
        return copy_value(config->cloak_key, sizeof(config->cloak_key), value);
    }
    ...
    STRING_OPTION("nickserv_db", nickserv_db);
    STRING_OPTION("chanserv_db", chanserv_db);
    STRING_OPTION("memoserv_db", memoserv_db);
    STRING_OPTION("history_db", history_db);
    STRING_OPTION("kline_default_reason", kline_default_reason);
    STRING_OPTION("zline_default_reason", zline_default_reason);
    STRING_OPTION("sendmail_path", sendmail_path);
    STRING_OPTION("mail_from", mail_from);
...
#undef STRING_OPTION
    ...
    if (strcmp(key, "max_channels") == 0) {
        if (number == 0UL || number > IRCD_HARD_MAX_CHANNELS) return -1;
        config->max_channels = (size_t)number;
        return 0;
    }
    if (strcmp(key, "max_connections_per_ip") == 0) {
        if (number > IRCD_HARD_MAX_CLIENTS) return -1;
        config->max_connections_per_ip = (size_t)number;
        return 0;
    }
    ...
    if (strcmp(key, "output_queue_max_bytes") == 0) {
        if (number < IRCD_OUTPUT_QUEUE_MIN_BYTES || number > IRCD_OUTPUT_QUEUE_HARD_MAX_BYTES)
            return -1;
        config->output_queue_max_bytes = (size_t)number;
        return 0;
    }
    if (strcmp(key, "history_limit") == 0) {
        if (number == 0UL || number > IRCD_HISTORY_HARD_LIMIT) return -1;
        config->history_limit = (size_t)number;
        return 0;
    }
    if (strcmp(key, "history_retention_days") == 0) {
        if (number > IRCD_HISTORY_RETENTION_HARD_MAX_DAYS) return -1;
        config->history_retention_days = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "history_max_rows") == 0) {
        if (number == 0UL || number > IRCD_HISTORY_MAX_ROWS_HARD_MAX) return -1;
        config->history_max_rows = (size_t)number;
        return 0;
    }
    if (strcmp(key, "channel_log_queue_max_rows") == 0) {
        if (number == 0UL || number > IRCD_CHANNEL_LOG_QUEUE_HARD_MAX_ROWS) return -1;
        config->channel_log_queue_max_rows = (size_t)number;
        return 0;
    }
    if (strcmp(key, "memoserv_quota") == 0) {
        if (number == 0UL || number > IRCD_MEMOSERV_QUOTA_HARD_MAX) return -1;
        config->memoserv_quota = (size_t)number;
        return 0;
    }
    if (strcmp(key, "memoserv_sender_quota") == 0) {
        if (number == 0UL || number > IRCD_MEMOSERV_SENDER_QUOTA_HARD_MAX) return -1;
        config->memoserv_sender_quota = (size_t)number;
        return 0;
    }
    if (strcmp(key, "memoserv_retention_days") == 0) {
        if (number > IRCD_MEMOSERV_RETENTION_HARD_MAX_DAYS) return -1;
        config->memoserv_retention_days = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nickserv_registrations_per_ip") == 0) {
        if (number > IRCD_NICKSERV_REGISTRATIONS_PER_IP_HARD_MAX) return -1;
        config->nickserv_registrations_per_ip = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nickserv_registration_window_seconds") == 0) {
        if (number < 60UL || number > IRCD_NICKSERV_REGISTRATION_WINDOW_HARD_MAX_SECONDS)
            return -1;
        config->nickserv_registration_window_seconds = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nickserv_mail_requests_per_ip") == 0) {
        if (number > IRCD_NICKSERV_MAIL_REQUESTS_PER_IP_HARD_MAX) return -1;
        config->nickserv_mail_requests_per_ip = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nickserv_mail_window_seconds") == 0) {
        if (number < 60UL || number > IRCD_NICKSERV_MAIL_WINDOW_HARD_MAX_SECONDS)
            return -1;
        config->nickserv_mail_window_seconds = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nickserv_mail_global_per_minute") == 0) {
    ...
    if (strcmp(key, "argon2_ops_per_ip") == 0) {
        if (number > IRCD_ARGON2_OPS_PER_IP_HARD_MAX) return -1;
        config->argon2_ops_per_ip = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "argon2_window_seconds") == 0) {
        if (number < 10UL || number > IRCD_ARGON2_WINDOW_HARD_MAX_SECONDS) return -1;
        config->argon2_window_seconds = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "argon2_global_ops_per_minute") == 0) {
        if (number > IRCD_ARGON2_GLOBAL_OPS_PER_MINUTE_HARD_MAX) return -1;
        config->argon2_global_ops_per_minute = (unsigned int)number;
        return 0;
    }
    ...
    if (strcmp(key, "dnsbl_timeout_seconds") == 0) {
        if (number == 0UL || number > 300UL) return -1;
        config->dnsbl_timeout_seconds = (unsigned int)number;
        return 0;
    }
    if (strcmp(key, "nospoof_timeout_seconds") == 0) {
        if (number < 5UL || number > 300UL) return -1;
        config->nospoof_timeout_seconds = (unsigned int)number;
        return 0;
    }
