...
int irc_mask_match(const char *pattern, const char *text) {
    ...
    while (*text != '\0') {
        ...
        if (*pattern == '?' || rfc1459_fold((unsigned char)*pattern) == rfc1459_fold((unsigned char)*text)) { ++pattern; ++text; continue; }
        if (star != NULL) { pattern = star; text = ++retry; continue; }
        return 0;
    }
...
static int mask_entry_matches_client(const ChannelMaskEntry *entry,
    ...
    (void)snprintf(identity, sizeof(identity), "%s!%s@%s", client->nick, client->user, client->display_host);
...
static int authorized_ban_matches_client(const ChannelMaskEntry *list,
    ...
    if (client == NULL) return 0;
    for (entry = list; entry != NULL; entry = entry->next)
        if (entry->protected_authorized && mask_entry_matches_client(entry, client)) return 1;
    return 0;
}

int channel_client_is_banned(const Channel *channel, const Client *client) {
    if (channel == NULL || client == NULL) return 0;
    return channel_mask_matches_client(channel->ban_list, client) && !channel_mask_matches_client(channel->exception_list, client);
}

int channel_client_is_banned_protected(const Channel *channel,
                                       const Client *client) {
    if (channel == NULL || client == NULL) return 0;
    return authorized_ban_matches_client(channel->ban_list, client) && !channel_mask_matches_client(channel->exception_list, client);
}

int channel_client_is_invex(const Channel *channel, const Client *client) {
    return channel != NULL && client != NULL && channel_mask_matches_client(channel->invite_exception_list, client);
}
...
int channel_invite_add(Channel *channel, uint64_t client_id) {
    ...
    for (invite = channel->invites; invite != NULL; invite = invite->next) {
        if (invite->client_id == client_id) return 0;
        ++count;
    }
    if (count >= IRC_CHANNEL_INVITE_MAX) return -2;
...
int channel_join_throttle_allows(Channel *channel, uint64_t client_id) {
    time_t now;
    (void)client_id;
    if (channel == NULL || channel->join_throttle_count == 0U || channel->join_throttle_seconds == 0U) return 1;
    now = time(NULL);
    if (channel->join_throttle_window_count == 0U || now < channel->join_throttle_window_start ||
        (unsigned long)(now - channel->join_throttle_window_start) >= (unsigned long)channel->join_throttle_seconds) return 1;
...
void channel_join_throttle_record(Channel *channel, uint64_t client_id) {
    time_t now;
    (void)client_id;
    if (channel == NULL || channel->join_throttle_count == 0U || channel->join_throttle_seconds == 0U) return;
    now = time(NULL);
    if (channel->join_throttle_window_count == 0U || now < channel->join_throttle_window_start ||
        (unsigned long)(now - channel->join_throttle_window_start) >= (unsigned long)channel->join_throttle_seconds) {
