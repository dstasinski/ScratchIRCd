/** @file dns.c @brief Non-blocking FCrDNS using a dedicated resolver thread. */
#include "dns.h"
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
static int write_full(int fd, const void *buffer, size_t length) {
    ...
    while (length > 0U) {
        ...
        cursor += (size_t)written; length -= (size_t)written;
    }
    return 0;
}
static int read_full(int fd, void *buffer, size_t length) {
    ...
    while (length > 0U) {
        ...
        cursor += (size_t)received; length -= (size_t)received;
    }
    return 1;
}
static int request_sockaddr(const DnsRequest *request, struct sockaddr_storage *storage,
                            socklen_t *length) {
    ...
    if (request->address_family == AF_INET) {
        ...
        *length = (socklen_t)sizeof(*address); return 0;
    }
...
static int address_matches(const DnsRequest *request, const struct sockaddr *address) {
...
static void resolve_request(const DnsRequest *request, DnsResult *result) {
    ...
    struct addrinfo *addresses = NULL, *candidate;
    char ptr_host[IRC_HOST_MAX + 1U];
    memset(result, 0, sizeof(*result));
    result->client_id = request->client_id;
    (void)snprintf(result->queried_ip, sizeof(result->queried_ip), "%s", request->ip);
    if (request_sockaddr(request, &storage, &length) != 0) return;
    if (getnameinfo((const struct sockaddr *)&storage, length, ptr_host, sizeof(ptr_host),
                    NULL, 0U, NI_NAMEREQD) != 0) return;
    memset(&hints, 0, sizeof(hints)); hints.ai_family = request->address_family; hints.ai_socktype = SOCK_STREAM;
    ...
    for (candidate = addresses; candidate != NULL; candidate = candidate->ai_next) {
        if (address_matches(request, candidate->ai_addr)) {
            result->verified = 1;
            (void)snprintf(result->resolved_host, sizeof(result->resolved_host), "%s", ptr_host);
            break;
        }
    }
...
int dns_resolver_init(DnsResolver *resolver) {
    ...
    if (pipe(requests) != 0 || pipe(results) != 0) {
        if (requests[0]>=0) close(requests[0]); if (requests[1]>=0) close(requests[1]);
        if (results[0]>=0) close(results[0]); if (results[1]>=0) close(results[1]); return -1;
    }
    ...
    if (set_nonblocking(resolver->request_write_fd)!=0 || set_nonblocking(resolver->result_read_fd)!=0 ||
        set_nonblocking(resolver->result_write_fd)!=0) { dns_resolver_destroy(resolver); return -1; }
    if (pthread_create(&resolver->thread,NULL,resolver_main,resolver)!=0) { dns_resolver_destroy(resolver); return -1; }
    resolver->running=1; return 0;
}
...
int dns_resolver_submit(DnsResolver *resolver, uint64_t client_id, int address_family, const char *ip) {
    DnsRequest request; ssize_t written;
    if (resolver == NULL || !resolver->running || ip == NULL) return -1;
    memset(&request,0,sizeof(request)); request.client_id=client_id; request.address_family=address_family;
    if (snprintf(request.ip,sizeof(request.ip),"%s",ip)<0) return -1;
    do { written=write(resolver->request_write_fd,&request,sizeof(request)); } while (written<0 && errno==EINTR);
    return written==(ssize_t)sizeof(request)?0:-1;
}
int dns_resolver_result_fd(const DnsResolver *resolver) { return resolver!=NULL?resolver->result_read_fd:-1; }
int dns_resolver_read_result(DnsResolver *resolver, DnsResult *result) {
    ...
    do { received=read(resolver->result_read_fd,result,sizeof(*result)); } while (received<0 && errno==EINTR);
