 ...
 *
 * Each result also carries the numeric IP that was queried. This is useful for
 * identity handoffs such as WebIRC, where a DNS request for the physical
 * gateway may already be in flight when the authenticated end-user IP becomes
...
/** Make one pipe endpoint non-blocking. */
...
/** Write a complete fixed-size request/result record. */
...
/** Read a complete fixed-size request record on the worker thread. */
...
/** Convert the numeric requested IP into a sockaddr for reverse lookup. */
...
/** Compare one forward-resolved address against the original numeric IP. */
...
/** Perform PTR lookup followed by forward confirmation for one request. */
static void resolve_request(const DnsRequest *request, DnsResult *result) {
    ...
    (void)snprintf(result->queried_ip, sizeof(result->queried_ip),
                   "%s", request->ip);
...
/** Resolver worker: consume requests serially and publish completed answers. */
